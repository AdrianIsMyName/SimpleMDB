...
public class DefaultActorsService : IActorsService
{
	...
	public DefaultActorsService(IActorsRepository actorRepository)
	{
	...
	public async Task<Result<Actor>> CreateActor(Actor newActor)
	{
		...
		return actor == null
				? new Result<Actor>(new Exception($"Could not create actor {newActor}."), (int)HttpStatusCode.NotFound)
	...
	public async Task<Result<Actor>> UpdateActor(int id, Actor newData)
	{
		...
		return actor == null
				? new Result<Actor>(new Exception($"Could not update actor {newData} with id {id}."), (int)HttpStatusCode.NotFound)
				: new Result<Actor>(actor, (int)HttpStatusCode.OK);
	}
	...
	private static Result<Actor>? ValidateActor(Actor? actorData)
	{
		...
		if (string.IsNullOrWhiteSpace(actorData.FirstName))
			return new Result<Actor>(new Exception("First name is required."), (int)HttpStatusCode.BadRequest);

		if (string.IsNullOrWhiteSpace(actorData.LastName))
			return new Result<Actor>(new Exception("Last name is required."), (int)HttpStatusCode.BadRequest);

		if (actorData.Rating < 0 || actorData.Rating > 10)
			return new Result<Actor>(new Exception("Rating must be between 0 and 10."), (int)HttpStatusCode.BadRequest);

		if (!string.IsNullOrWhiteSpace(actorData.Bio) && actorData.Bio.Length > 1024)
			return new Result<Actor>(new Exception("Bio cannot exceed 1024 characters."), (int)HttpStatusCode.BadRequest);
